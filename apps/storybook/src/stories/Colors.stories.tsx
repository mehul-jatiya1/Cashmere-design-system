...
const label = (s: string) => (
  <h3 style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 11, fontWeight: 600, letterSpacing: '0.08em', textTransform: 'uppercase', color: '#767676', margin: '24px 0 8px' }}>
    {s}
  </h3>
)

interface SwatchProps { name: string; value: string; light?: boolean }
const Swatch = ({ name, value, light }: SwatchProps) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 4, width: 80 }}>
    <div style={{ width: 80, height: 48, borderRadius: 6, background: value, border: '1px solid rgba(0,0,0,0.08)' }} />
    <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 10, fontWeight: 600, color: '#1b1b1b', lineHeight: 1.4 }}>{name}</span>
    <span style={{ fontFamily: 'DM Sans, sans-serif', fontSize: 10, color: '#767676', lineHeight: 1.4 }}>{value}</span>
  </div>
)
    ...
    <div style={{ padding: 8 }}>
      ...
      <Row>
        {[
          ...
          ['700', '#bbbbbb'], ['800', '#d1d1d1'], ['900', '#e8e8e8'], ['950', '#f2f2f2'], ['1000', '#fffffc'],
        ].map(([n, v]) => <Swatch key={n} name={`neutral-${n}`} value={v} />)}
      </Row>
      ...
      <Row>
        ...
        ].map(([n, v]) => <Swatch key={n} name={`accent-${n}`} value={v} />)}
      </Row>
      ...
      <Row>
        ...
        ].map(([n, v]) => <Swatch key={n} name={`positive-${n}`} value={v} />)}
      </Row>
      ...
      <Row>
        ...
        ].map(([n, v]) => <Swatch key={n} name={`negative-${n}`} value={v} />)}
      </Row>
      ...
      <Row>
        ...
        ].map(([n, v]) => <Swatch key={n} name={`warning-${n}`} value={v} />)}
      </Row>
    </div>
  ),
}
